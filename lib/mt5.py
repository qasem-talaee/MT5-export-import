...
class MT5:
    ...
    def Import(self, filename):
        ...
        with open(filename, 'r', newline='') as f:
            ...
            for row in reader:
                if row[0] != "Symbol":
                    if len(mt5.orders_get(symbol=row[0])) == 0:
                        if row[6] != "0":
                        ...
                        if result is None:
                            log_list.append(f"Failed to send order for {row[0]}: No response from MT5")
                            log_list.append(f"This Symbol is not recreated : {row[0]}")
                        elif result.retcode != mt5.TRADE_RETCODE_DONE:
                            log_list.append(f"Failed to recreate order for {row[0]}: {result.comment}")
                            log_list.append(f"This Symbol is not recreated : {row[0]}")
                        else:
                            log_list.append(f"Order recreated for {row[0]}")
